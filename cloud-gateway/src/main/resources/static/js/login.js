/** * Registration form */$('#signup').submit(function (e) {
    e.preventDefault();
...
/** * E-mail form */$('#mail').submit(function (e) {
    e.preventDefault();
    ...
    if (email) {
        $.ajax({
            ...
            data: JSON.stringify({
                email: email,
                scheduledNotifications: {"REMIND": {"active": true, "frequency": "MONTHLY"}}
            }),
...
/** * Demo */$(".demobutton").bind("click", function () {
    $.ajax({
        url: 'accounts/demo', datatype: 'json', type: 'get', async: false, success: function (data) {
...
/** * Login form effects */function initialShaking() {
    autoShake();
...
$(".fliptext").bind("click", function () {
    setTimeout(function () {
        $("#plusavatar").addClass("avataranimation");
    }, 1000);
